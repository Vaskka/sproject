...
{
	m_WhiteNoiseTex = util::loadTexture(WHITE_NOISE_TEXTURE_PATH.c_str(), GL_REPEAT, GL_LINEAR, GL_RED);
	m_AbstractNoiseTex01 = util::loadTexture(ABSTRACT_NOISE_01_TEXTURE_PATH.c_str(), GL_REPEAT, GL_LINEAR, GL_RGB);
...
void CYGGRenderer::__renderTerrainPass()
{
	...
	glBindTexture(GL_TEXTURE_2D, m_AbstractNoiseTex01);
...
void CYGGRenderer::__playSound()
{
	...
	sengine::audioEngine::playAudio2D(Constant::MAIN_BGM_FILE_PATH[RandInt].c_str());
}
